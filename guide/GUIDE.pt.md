...
O AS64500 é multihomed, e tem uma preferência: **o Provedor B é a entrada, o Provedor A
é o backup.** Para isso, a origem faz *prepend* do seu próprio ASN duas vezes ao anunciar
para o Provedor A (`64500 64500 64500` em vez de apenas `64500`), então todo caminho por
...
| peer | 64999 | uma rede legítima que faz peering com a origem e compra trânsito do Provedor A |

Os ASNs 64496–64511 são ASNs reservados para documentação (RFC 5398). O AS64999 está,
em vez disso, na faixa de uso privado (RFC 6996), e o AS666 não está em nenhuma das duas - é só
...
4. Confira se os roteadores subiram e se as sessões BGP estão estabelecidas.
   ...
   `attacker_v6`, as sessões com o AS666, que está no ar mas em silêncio por enquanto.
...
e o registro, no painel do LabNIC. É a mesma troca de XML que
acontece entre um provedor e o seu RIR.

### O seu lado: a CA no Krill

1. Abra o Krill: **https://localhost:3000**

   O navegador vai reclamar do certificado autoassinado. Clique em
   "Advanced" e prossiga.
...
4. Em outra aba, abra o painel do registro: **http://localhost:8081**
...
12. No Krill, a CA deve mostrar os recursos recebidos do pai: o
    ASN 64500 e os prefixos 10.0.0.0/24 e 3fff:cafe::/32.
...
1. Abra o Krill em **https://localhost:3000**, entre com o token
...
2. Confira se o prefixo da origem chega aos dois observadores, pelos dois
   ...
   *     N-? 10.0.0.0/24          10.200.5.10       100     0 64501 64500 64500 64500 i
   ...
   Dois caminhos em cada observador: `64502 64500` (o selecionado: a entrada preferida da origem, 2
   saltos) e `64501 64500 64500 64500` (o backup, mantido na tabela mas com 4 saltos
...
3. **Dê uma olhada em como esses roteadores estão configurados.** Abra os
   ...
   Você vai encontrar BGP comum: sessões com os dois provedores (e com o AS666,
...
2. **Antes de olhar:** o caminho do AS666 é apenas `666` - mais curto que o
   legítimo `64502 64500` (e que o backup por A). Qual você espera que os observadores prefiram?
...
3. Agora olhe:
   ...
   **âmbar**: um observador está aceitando o que ele anuncia. A tabela de *Vereditos*
   tem novas linhas rotuladas *AS666*, sem vereditos, porque não há
   validação.
...
3. Crie a ROA IPv6:
   ...
   `krillc roas update --add "10.0.0.0/24-24 => 64500"`, e o mesmo com
...
3. Olhe de novo: o sequestro sumiu da tabela. Não está perdido -
   ...
   O selo agora diz *ROV: descartando*. O sequestro foi derrotado. O AS666 continua
...
> **Estado:** estágio `rov-drop` · AS666 fazendo o sequestro ingênuo (descartado) · peer
...
1. Passe o AS666 para o ataque de caminho forjado:
   ...
   O AS666 agora anuncia o caminho `666 64500`: como se tivesse recebido o
   prefixo diretamente da origem verdadeira.

   **Essa relação é mentira.** O AS666 não tem sessão BGP - nem peering, nem
   trânsito, nada - com o AS64500: os dois nem estão conectados. A
   adjacência `666 64500` existe só porque a configuração do AS666 a *escreve no
   ...
   filtro, então o caminho é só `666`), e confira que nenhum dos dois arquivos tem uma
...
3. Olhe:
   ...
   *     V-? 10.0.0.0/24          10.200.5.10       100     0 64501 64500 64500 64500 i
   ...
   - o caminho termina em 64500, que é exatamente o que a ROA autoriza. Os
   ...
   > O caminho forjado (`666 64500`, 2 saltos) empata com o do Provedor B (`64502 64500`, também 2),
...
> **Estado:** estágio `rov-drop` · AS666 forjando o caminho · peer em silêncio · ROAs para
...
3. Crie o objeto ASPA declarando quais provedores podem propagar rotas do
   AS64500. Por causa da história, liste **só o Provedor A** por enquanto - você vai
...
> endereços (`AS64501(v4)`), mas a versão final do perfil ASPA no
...
3. Olhe as rotas:
   ...
   *     V-! 10.0.0.0/24          10.200.6.10        20     0 64502 64500 i
   ...
   O caminho forjado agora é **ASPA Invalid** - o salto `64500 → 666` não está
   autorizado, já que só 64501 está listado. Ele ainda é visível, mas com
...
Olhe o `show rib` de novo. **Duas** rotas sumiram, não uma. Uma é o caminho forjado do AS666.
...
> **Estado:** estágio `aspa-drop` · AS666 forjando o caminho · peer em silêncio · ROAs para
...
2. Confira os observadores:
   ...
   *     V-V 10.0.0.0/24          10.200.5.10       200     0 64501 64500 64500 64500 i
   ...
   o AS666 - ainda anunciando - continua derrotado. Repare como o conserto não
   envolveu o AS666 em nada: o objeto ASPA descreve as *suas* relações, e
...
Deixe o AS666 rodando. Ele é inofensivo agora, e será um lembrete útil no
...
> **Estado:** estágio `aspa-drop` · AS666 forjando o caminho (derrotado) · peer em silêncio
...
1. Para ver os vereditos, ponha a verificação ASPA de volta no estágio de marcação - uma
   ...
   *     V-! 10.0.0.0/24          10.200.5.10        20     0 64501 64999 64500 i
   ...
   Provedor A: `64501 64999 64500`. Duas coisas a notar:
   ...
   - **ASPA Invalid.** O salto `64500 → 64999` nunca foi autorizado: o
...
ao começo de tudo: ele silencia o AS666 e o peer, *e* tira a validação
dos observadores.)
...
| Vazamento de rota (AS_PATH `64501 64999 64500`) | **não enxerga** (a origem é genuína) | **pega** (o salto `64500 → 64999` não está autorizado) |
...
3. Agora faça o equivalente no BIRD: troque `aspa_check_upstream` por
   ...
   **Antes de olhar:** você espera que o novo veredito do observer1 coincida com o
   ...
   (`(64510, 2, 1)`). As duas leituras são bem mais permissivas que o algoritmo
...
6. Mais um caso de borda, já que você está aqui: passe o AS666 de volta para o seu
   ...
   esse caminho de `Valid` (`(64510, 2, 2)`), o OpenBGPD de `Unknown` (`?`). De novo duas
...
`bgp_path: 64502 64500` - um caminho
perfeitamente legítimo por um provedor autorizado - mas **ROV
Invalid**: a ROA de `10.0.0.0/24` só autoriza anúncios até
...
1. Abra o Routinator: **http://localhost:8323**
   ...
   ele vem da âncora de confiança do próprio LabNIC (e está disponível para download
...
3. Confira os vereditos de `10.0.0.0/24` nos dois observadores: os dois caminhos devem
   ...
   que `3fff:cafe::/32` não é afetado: a ROA dele ainda está lá, então ele
...
| A rota do AS666 não aparece | Quando um observador está *descartando* o que uma verificação sinaliza (`rov-drop`, `aspa-drop`), essa rota some da tabela de propósito: olhe em `birdc show route table master4 filtered` no observer1. Em `none` não há vereditos, mas a rota deve estar lá. Caso contrário, confira se você rodou o comando do passo e se a sessão está no ar: `docker exec lab-attacker birdc show protocols`. |
