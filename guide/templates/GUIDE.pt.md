...
É isso que estes termos significam *neste laboratório*, não um tratado
completo sobre o assunto. Pule à frente se já está confortável com eles.
...
(`{{ORIGIN_ASN}} {{ORIGIN_ASN}} {{ORIGIN_ASN}}` em vez de apenas `{{ORIGIN_ASN}}`), então
...
com uma única exceção: o AS{{ATTACKER_ASN}}, que não está em nenhum bloco
...
os observadores estão, o que o AS{{ATTACKER_ASN}} e o peer estão fazendo, e
...
- **O AS{{ATTACKER_ASN}} e o peer.** Todo comando `stepN-*` também define o estado
  ...
  continuam colocando o AS{{ATTACKER_ASN}} de volta na forma de caminho forjado, já
...
1. Requisitos: Docker instalado (Mac, Windows ou Linux, com **OrbStack**,
...
4. Confira se os roteadores subiram e se as sessões BGP estão estabelecidas.
   ...
   `attacker_v6`, as sessões com o AS{{ATTACKER_ASN}}, que está no ar mas em
...
registro, no painel do {{RIR_NAME}}. É a mesma troca de XML que acontece
...
> `./scripts/lab.sh reset`, depois `up`, depois a Preparação 2 de novo, rodando
> tudo pelo terminal do seu próprio computador, não pelo painel.
...
2. Confira se o prefixo da origem chega aos dois observadores pelos dois
   ...
   *     N-? {{ORIGIN_V4}}          10.200.5.10       100     0 {{PROVIDER_A_ASN}} {{ORIGIN_ASN}} {{ORIGIN_ASN}} {{ORIGIN_ASN}} i
   ...
   Dois caminhos em cada observador: `{{PROVIDER_B_ASN}} {{ORIGIN_ASN}}` (o
   selecionado, a entrada preferida da origem, 2 saltos) e
   `{{PROVIDER_A_ASN}} {{ORIGIN_ASN}} {{ORIGIN_ASN}} {{ORIGIN_ASN}}` (o backup,
...
3. **Dê uma olhada em como esses roteadores estão configurados.** Abra os
   ...
   AS{{ATTACKER_ASN}}, calado por enquanto), e uma política que aceita tudo:
...
2. **Antes de olhar:** o caminho do AS{{ATTACKER_ASN}} é apenas `{{ATTACKER_ASN}}`, mais
   curto que o legítimo `{{PROVIDER_B_ASN}} {{ORIGIN_ASN}}` (e que o backup por
...
3. Agora olhe:
   ...
   *     N-? {{ORIGIN_V4}}          10.200.5.10       100     0 {{PROVIDER_A_ASN}} {{ORIGIN_ASN}} {{ORIGIN_ASN}} {{ORIGIN_ASN}} i
   ...
   rotuladas *AS{{ATTACKER_ASN}}*, sem vereditos, porque ainda não há validação.
...
O AS{{ATTACKER_ASN}} lê a mesma documentação que você. O ROV só olha o
...
1. Passe o AS{{ATTACKER_ASN}} para o ataque de caminho forjado:
   ...
   O AS{{ATTACKER_ASN}} agora anuncia o caminho `{{ATTACKER_ASN}} {{ORIGIN_ASN}}`, como se
   tivesse recebido o prefixo diretamente da origem verdadeira.

   **Essa relação é mentira.** O AS{{ATTACKER_ASN}} não tem sessão BGP nenhuma com
   o AS{{ORIGIN_ASN}}: nem peering, nem trânsito, nada. Os dois nem estão
   conectados. A adjacência `{{ATTACKER_ASN}} {{ORIGIN_ASN}}` existe só porque a
   configuração do AS{{ATTACKER_ASN}} a *escreve no caminho*. Veja como:
   ...
   `{{ATTACKER_ASN}}`), e confira que nenhum dos dois arquivos tem uma sessão
...
3. Olhe:
   ...
   *     V-? {{ORIGIN_V4}}          10.200.5.10       100     0 {{PROVIDER_A_ASN}} {{ORIGIN_ASN}} {{ORIGIN_ASN}} {{ORIGIN_ASN}} i
   ...
   o caminho termina em {{ORIGIN_ASN}}, exatamente o que a ROA autoriza. Os
   ...
   > O caminho forjado (`{{ATTACKER_ASN}} {{ORIGIN_ASN}}`, 2 saltos) empata com
   > o do Provedor B (`{{PROVIDER_B_ASN}} {{ORIGIN_ASN}}`, também 2), e o
...
3. Crie o objeto ASPA declarando quais provedores podem propagar rotas do
   AS{{ORIGIN_ASN}}. Por causa da história, liste **só o Provedor A** por
...
> de endereços (`AS{{PROVIDER_A_ASN}}(v4)`), mas a versão final do perfil
...
3. Olhe as rotas:
   ...
   *     V-! {{ORIGIN_V4}}          10.200.6.10        20     0 {{PROVIDER_B_ASN}} {{ORIGIN_ASN}} i
   ...
   O caminho forjado agora é **ASPA Invalid**: o salto `{{ORIGIN_ASN}} →
   {{ATTACKER_ASN}}` não está autorizado, já que só {{PROVIDER_A_ASN}} está
...
2. Confira os observadores:
   ...
   *     V-! {{ORIGIN_V4}}          10.200.8.10        20     0 {{ATTACKER_ASN}} {{ORIGIN_ASN}} i
   ...
   rebaixado (`V-!`, 20), e o AS{{ATTACKER_ASN}}, ainda anunciando, continua
   derrotado. Repare como o conserto não envolveu o AS{{ATTACKER_ASN}} em nada:
...
Deixe o AS{{ATTACKER_ASN}} rodando. Ele é inofensivo agora, e serve de lembrete
...
4. Agora os observadores:
   ...
   **O caminho próprio do Provedor A já sumiu.** Ele parou de anunciá-lo no
   ...
   `{{PROVIDER_A_ASN}} {{PEER_ASN}} {{ORIGIN_ASN}}`, e como marcar nunca remove
   ...
   - **ASPA Invalid.** O salto `{{ORIGIN_ASN}} → {{PEER_ASN}}` nunca foi
...
3. Olhe as rotas de novo:
   ...
   **Duas rotas desapareceram, não uma.** O caminho forjado do
   AS{{ATTACKER_ASN}} sumiu, como esperado. O mesmo aconteceu com a entrada
...
AS{{ATTACKER_ASN}} e o peer, *e* tira a validação dos observadores.)
...
3. Agora faça o equivalente no BIRD: `bird/observer1-extra-a-downstream.conf` é
   ...
   **ASPA Unknown** (`({{OBSERVER1_ASN}}, 2, 1)`). As duas leituras são bem
...
Um exemplo em IPv4 significaria desagregar {{ORIGIN_V4}} até um `/25`, algo
...
provedores, ele usa **dois** sub-blocos de {{ORIGIN_V6}}, um anunciado só ao
...
(ambas dentro de {{ORIGIN_V6}}, ambas mais específicas do que
{{ORIGIN_V6_MAXLEN}} autoriza), e o filtro de exportação de cada provedor
...
1. Abra o Routinator: **http://routinator.localhost:8080**
   ...
   `MODE=local` ele vem da âncora de confiança do próprio {{RIR_NAME}} (e
...
3. Confira os vereditos de `{{ORIGIN_V4}}` nos dois observadores: os dois
   ...
   com as duas verificações ligadas. Repare que `{{ORIGIN_V6}}` não é
